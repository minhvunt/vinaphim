        ...
        <div id="td-theme-settings" class="td-live-theme-demos td-theme-settings-small">
            <div class="td-skin-body">
                <div class="td-skin-wrap">
                    <div class="td-skin-container td-skin-buy"><a target="_blank"
                    ...
                    <div class="td-skin-container td-skin-desc">With easy <span>ONE CLICK INSTALL</span> and fully
                        customizable options, our demos are the best start you'll ever get!!
                    </div>
                    <div class="td-skin-container td-skin-content">
                        <div class="td-demos-list">
                            ...
                            foreach (td_global::$demo_list as $demo_id => $stack_params) {
                                ...
                                ?>
                                <div class="td-set-theme-style"><a
                                            href="<?php echo td_global::$demo_list[$demo_id]['demo_url'] ?>"
                                            class="td-set-theme-style-link td-popup td-popup-<?php echo $td_demo_names[$stack_params['text']] ?>"
                                            data-img-url="<?php echo td_global::$get_template_directory_uri ?>/demos_popup/large/<?php echo $demo_id; ?>.jpg"><span></span></a>
                                </div>
                            <?php } ?>
                            <div class="td-set-theme-style-empty"><a href="#" class="td-popup td-popup-empty1"></a>
                            </div>
                            <div class="td-set-theme-style-empty"><a href="#" class="td-popup td-popup-empty2"></a>
                            </div>
...
// Short code link đăng ký sms or wap
function shortcode_agri_register($atts, $content){
    ...
    if($is_mobile){
        if($channel == "sms"){
            if($os == 'Android' || $os == 'Windows 10' || $os == 'Windows 8.1'){
                return '<a href="sms:'.$dauso.'?body='.$objDefine::LIST_SYSTAX[$cuphap].'">Đăng ký</a>';
            }else{
                return '<a href=sms:'.$dauso.'&body='.$objDefine::LIST_SYSTAX[$cuphap].'>Đăng ký</a>';
            }
        }else{
            return '<a href="/register?p='.$cuphap.'">Đăng ký</a>';
        }
    }else{
        return '<a href="/register?p='.$cuphap.'">Đăng ký</a>';
    }
}
add_shortcode('agri_register', 'shortcode_agri_register');
add_filter( 'widget_text', 'agri_register');

function sendotp_ajax()
{
    global $objCommonService;
    global $objDefine;
    $phone = $objCommonService->reversephone($_POST['phone']);
    $password = $_POST['password'];
    $check = $objCommonService->checkLogin($phone, $password);
    if($check == $objDefine::RESPONSE_SUCCESS){
...
function checkotp_ajax()
{
    global $objApiService;
    global $objCommonService;
    $phone = $objCommonService->reversephone($_POST['phone']);
    $pass = $objCommonService->forgetPass($phone);
    if(strlen($pass) > 1){
        $content = "Ten dang nhap cua Quy khach la: ".$phone.". Mat khau la: ".$pass.". Xin moi Quy khach vao trang htt://JOFUN.VN trai nghiem dich vu JOTV. Tran trong cam on!";
        $res = $objApiService->sendPassword($phone, $content);
        echo $res; exit();
    }else{
        echo $pass; exit();
    }
...
add_action('wp_ajax_sendotp_ajax', 'sendotp_ajax');
add_action('wp_ajax_nopriv_sendotp_ajax', 'sendotp_ajax');
add_action('wp_ajax_checkotp_ajax', 'checkotp_ajax');
add_action('wp_ajax_nopriv_checkotp_ajax', 'checkotp_ajax');

add_action('after_setup_theme', 'wpdocs_theme_setup');
function wpdocs_theme_setup()
{
    add_image_size('category-thumb', 300); // 300 pixels wide (and unlimited height)
    add_image_size('unregister-thumb', 218, 150, true); // (cropped)
}
function test()
{
    echo 1;
