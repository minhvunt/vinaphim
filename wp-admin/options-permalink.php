...
if ( $iis7_permalinks ) {
	...
	if ( ( ! file_exists( $home_path . '.htaccess_bak' ) && is_writable( $home_path ) ) || is_writable( $home_path . '.htaccess_bak' ) ) {
		$writable = true;
	} else {
		$writable = false;
		$existing_rules  = array_filter( extract_from_markers( $home_path . '.htaccess_bak', 'WordPress' ) );
...
if ( isset($_POST['permalink_structure']) || isset($_POST['category_base']) ) {
	...
	if ( $iis7_permalinks ) {
		...
		$message = __( 'You should update your .htaccess_bak now.' );
	}
...
<p><?php
	printf(
		/* translators: 1: .htaccess_bak, 2: Codex URL, 3: CTRL + a */
